...
class ShangchangController extends PublicController {
    ...
    public function shop_details(){
    	...
    	$shop_info = M('shangchang')->where('id='.intval($shop_id))->field('id,name,uname,tel,utel,vip_char,address_xq,content')->find();
    	if (!$shop_info) {
    	...
    	$content = str_replace("/minibuxiugangjyw/Data/", __DATAURL__, $shop_info['content']);
        $content=html_entity_decode($content, ENT_QUOTES ,'utf-8');
    	echo json_encode(array('status'=>1,'shop_info'=>$shop_info,'content'=>$content));
    	exit();
    }
